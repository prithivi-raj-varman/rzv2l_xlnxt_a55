...
// #if defined(__linux__)
// #define INT_MHU_RSP_CH1_NS (107)
// #endif
// #define INT_MBX_A55 (INT_MHU_RSP_CH1_NS)
// #define INT_MBX_M33 (INT_MHU_MSG_CH1_NS)
 ...
 * Page size on Linux
 * -------------------------------------------------------------------------- */
#define PAGE_SIZE (0x01000U) /* 4KB page size */
...
/* metal_device_open() register map */
#define MBX_REG_BASE    (MHU_REG_BASE)
#define MBX_REG_SIZE    (MHU_MAP_SIZE)
#define MBX_MAP_SIZE    (PAGE_SIZE)
...
#define MBX_SEND_TYPE_MSG_CHANNEL_MASK (0xAU)

#define MBX_LOCAL_INT_STS_REG(y)  \
    (((1 << (y)) & MBX_SEND_TYPE_MSG_CHANNEL_MASK) ? \
     (MBX_REG_START + (y) * 0x20U + !MBX_LOCAL * 0x10U) : \
     (MBX_REG_START + (y) * 0x20U + MBX_LOCAL * 0x10U))

#define MBX_REMOTE_INT_STS_REG(y) \
    (((1 << (y)) & MBX_SEND_TYPE_MSG_CHANNEL_MASK) ? \
     (MBX_REG_START + (y) * 0x20U + !MBX_REMOTE * 0x10U) : \
     (MBX_REG_START + (y) * 0x20U + MBX_REMOTE * 0x10U))
...
#define SHM_LOCAL_OFFSET(y)  \
    (((1 << (y)) & MBX_SEND_TYPE_MSG_CHANNEL_MASK) ? \
     (0x08U*(y) + 0x04U * MBX_LOCAL) : \
     (0x08U*(y) + 0x04U * MBX_REMOTE))

#define SHM_REMOTE_OFFSET(y) \
    (((1 << (y)) & MBX_SEND_TYPE_MSG_CHANNEL_MASK) ? \
     (0x08U*(y) + 0x04U * MBX_REMOTE) : \
     (0x08U*(y) + 0x04U * MBX_LOCAL))

/* --------------------------------------------------------------------------
 * RPMSG / Remoteproc definitions
 * -------------------------------------------------------------------------- */
#define RPVDEV_MAX_NUM (MBX_MAX_CHN)
...
/* --------------------------------------------------------------------------
 * Platform API declarations (C linkage)
 * -------------------------------------------------------------------------- */
...
void platform_release_rpmsg_vdev(struct remoteproc *platform,
